...
import main.securityincident.controller.CompanyController;
import main.securityincident.model.dto.CompanyDto;
    ...
    public void companyFindOne(){
        ...
        for(;;){
            try {
                ...
                System.out.print("기업ID 입력 > ");                int companyId = scan.nextInt();
                ArrayList<CompanyDto> companyDtos = cc.companyFindOne(companyId);
                for(CompanyDto list: companyDtos) {
                    System.out.printf(
                            "기업ID   : %d\n기업명   : %s\n산업군   : %s\n본사위치 : %s\n설립연도 : %s\n\n보안 사고 발생 건수 : %s건\n최근 사고 발생일   : %s\n\n",
                            list.getCompanyId(), list.getCompanyName(), list.getIndustryIdName(), list.getHeadOffice(), list.getFoundedYear(),list.getIncidentCount(),list.getLastDate()
                    );
                }
                System.out.println("======================== 다음 동작 선택 ========================");
